...
// Regression tests for the "Open Here" action crash fix: GetOmniboxEventResultType
// must not hit NOTREACHED() when the match has an OpenHereAction.
TEST_F(BraveAutocompleteResultTest,
  ...
  EXPECT_EQ(result_type,
            metrics::OmniboxEventProto::Suggestion::HISTORY_TITLE);
}
